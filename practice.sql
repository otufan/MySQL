...
update worker
set worker_salary=(select max(worker_salary)from (select worker_salary from worker) wr)*0.8
where worker_name='veli han';

-- en dusuk maasi yuzde %30 arttirin ve ali can beyin mevcut maasi ile update edin

update worker
set worker_salary=(select min(worker_salary) from (select worker_salary from worker) wr)*1.3
...
where worker_salary<(select avg(worker_salary) from (select worker_salary from worker) wr);

-- Maaşın ortalama maaştan az olması durumunda maaşları ortalama maaşa eşit

update worker
set worker_salary=(select avg(worker_salary) from (select worker_salary from worker) wr)
where worker_salary<(select avg(worker_salary) from (select worker_salary from worker) wr);
